...
_RAND_COLOR_INCREASING_TRANSFORMS = [
    'AutoContrast',
    'Equalize',
    'Invert',
    'PosterizeIncreasing',
...
def rand_augment_transform_from_str(
    config_str, hparams=None, transforms=_RAND_TRANSFORMS
):
    """
    ...
    hparams = hparams or {'magnitude_std': 0.5}
    config = config_str.split('-')
    assert config[0] in ['rand', 'rand_color']
    config = config[1:]
    for c in config:
        cs = re.split(r'(\d.*)', c)
        if len(cs) < 2:
...
def rand_augment_transform(
    name, num_layers=2, magnitude=10, mstd=0.5, weight_idx=None, blacklist=None
):
    """
    Create a RandAugment transform
    """
    hparams = {'magnitude_std': mstd}
    is_color = 'color' in name
    is_inc = 'inc' in name
    transform_opts = {
        True: {True: _RAND_COLOR_INCREASING_TRANSFORMS, False: _RAND_COLOR_TRANSFORMS},
        False: {True: _RAND_INCREASING_TRANSFORMS, False: _RAND_TRANSFORMS},
    }
    transforms = transform_opts.get(is_color, False).get(is_inc, False)
    if blacklist is not None:
        transforms = [x for x in transforms if x not in blacklist]
